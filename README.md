# Hi — I'm Anshuman 👋

## 👨‍💻 Cybersecurity Projects

- **Real-Time Cyber Attack Detection & Analysis using Azure Sentinel + KQL** 🚀🔍  
  [Read the project →](https://github.com/anshuman-dev-seekhoon/Azure-Sentinel-SIEM-Lab/blob/main/README.md)  
  *SIEM lab with detection rules, KQL queries and interactive workbooks showing simulated incidents.*

- **Network Traffic Logging & Secure Communication Analysis Tool** 🔐  
  [Repo →](https://github.com/anshuman-dev-seekhoon/Network-Traffic-Loggingtool)  
  *TCP/IP capture and analysis toolkit using tcpdump and Python for parsing and alerting.*

- **SniffGuardX — Lightweight NIDS (Python)**  
  [Repo →](https://github.com/anshuman-dev-seekhoon/sniffguardx)  
  *Custom network intrusion detection prototype built with Scapy, with signature and heuristic detection.*

- **File Integrity Monitor (FIM) with Email Alerts**  
  [Repo →](https://github.com/anshuman-dev-seekhoon/FileIntegrityMonitor)  
  *Monitors sensitive directories via SHA-256 checksums and sends email alerts on modifications.*

- **Educational Ransomware (simulation for learning only)** ⚠️  
  [Repo →](https://github.com/anshuman-dev-seekhoon/ransomware)  
  *Created strictly for controlled lab experiments and defensive training. Not for malicious use.*

---

## 🛡️ SOC Case Studies & Incident Analysis

I maintain a dedicated repository where I document SOC-style incident investigations and security case studies that I’ve personally worked through during hands-on training and lab environments.

These are not tool walkthroughs or copy-pasted lab solutions. Each writeup focuses on:
- how an incident initially appeared,
- what indicators actually mattered,
- where assumptions or controls were misleading,
- and what I would improve from a detection and response perspective.

### 📂 Featured Case Studies

- **Phishing Investigation: When SPF Passes and Everything Looks Fine**  
  *Targeted executive phishing using a typo-squatted domain that passed SPF validation.*  
  → Focus: email header analysis, trust assumptions, executive risk, detection gaps  
  [Read the case study →](https://github.com/anshuman-dev-seekhoon/mycyber-casestudies/blob/main/phishing-spf-bypass/README.md)
- **DarkTortilla RAT Investigation: From Initial Access to Credential Theft**
  *Evidence-driven network forensic investigation tracing a multi-stage malware attack using PCAP analysis while distinguishing confirmed observations from analyst inferences.*
  → Focus: Network forensics, IOC extraction, C2 analysis, PowerShell, Mimikatz, FTP exfiltration, MITRE ATT&CK, detection opportunities
  [Read the case study →](https://github.com/anshuman-dev-seekhoon/mycyber-casestudies/tree/main/darktortilla-multi-stage-malware-investigation)
- *(More SOC case studies in progress)*

🔗 **All case studies:**  
https://github.com/anshuman-dev-seekhoon/mycyber-casestudies


---

## 🎓 Internship and Certifications

- [CompTIA Security+ (Credly)](https://www.credly.com/badges/491fde73-03a3-4983-ba34-bc99d948219e/public_url)  
- [Google Cybersecurity Professional Certificate (Credly)](https://www.credly.com/badges/cec10266-f53c-43f0-83c1-40fe1cf7e50d/public_url)  
- [Practical SOC Analyst Associate — TCM Security (Verification)](https://certified.tcm-sec.com/8cb8f799-869b-4216-95dc-604f4106a246?key=1f9e55d59f992090f7d1dd57e6dbae6ef9e18e98a43fe29de1eb97dbb370bb3e)
-  [CyArt: SOC Intern(Internship certificate)](https://github.com/anshuman-dev-seekhoon/anshuman-dev-seekhoon/blob/main/anshuman_cyart.pdf)  

> **Note:** Full verification links and badge images are available in the `certificates/` folder of this repo (recommended).

---

## 🧭 Roadmaps & Talks

- [📺 My Cybersecurity Roadmap (YouTube)](https://youtu.be/ZwsJFGcNFYU?si=UsEl16IzIuKgj-jW)  
  *An overview of my personal cybersecurity learning and development strategy (not a project demo).*

---

## 🔗 Other Links

- GitHub: [github.com/anshuman-dev-seekhoon](https://github.com/anshuman-dev-seekhoon)  
- LinkedIn: [linkedin.com/in/anshuman-singh-46b97131b](https://www.linkedin.com/in/anshuman-singh-46b97131b/)

---

## 🤝 Connect with me

- Email: [sanshuman066@gmail.com](mailto:sanshuman066@gmail.com)
- LinkedIn: [linkedin.com/in/anshuman-singh-46b97131b](https://www.linkedin.com/in/anshuman-singh-46b97131b/)

---

